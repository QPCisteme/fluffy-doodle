                      ...
                      uint8_t *data, uint8_t size)
{
    ...
    printk("TX : ");
    for (int i = 0; i < 4 * word_number + 6; i++)
        printk("%.2x ", tx_full[i]);
    printk("\r\n");
                      ...
                      uint8_t tx_size, uint8_t *rx, uint8_t rx_size, bool write)
{
    ...
    printk("TX : %.4x %.4x ", sys_get_be16(tx_header),
           sys_get_be16(tx_header + 2));
    for (int i = 0; i < tx_size; i++)
        printk("%.2x ", tx[i]);
    printk("\r\n");

    printk("RX: %.4x %.4x ", header, events);
...
static void wq_get_event(struct k_work *work)
{
    ...
    const struct mpl460a_config *drv_config = data->dev->config;
